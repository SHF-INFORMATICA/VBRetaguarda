...
Begin VB.Form frmPEDIDOTEMP 
   ...
   End
End
Attribute VB_Name = "frmPEDIDOTEMP"
...
'On Error GoTo ERRO_TRATA
   ...
   NUMR_COMANDA_N = 0 & Trim(txtComanda.Text)

   If TabComanda.State = 1 Then _
      TabComanda.Close

   SQL = "select * from CARTAOBARRA WITH (NOLOCK)"
   SQL = SQL & " where CARTAOBARRA_id = " & NUMR_COMANDA_N
