...
Begin VB.Form frmProducaoRegistroProducaoConsulta 
   Caption         =   "Consulta Registro Produção"
   ClientHeight    =   6840
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   10950
   ...
   EndProperty
   Icon            =   "INDRegistroProducaoConsulta.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   ScaleHeight     =   6840
   ScaleWidth      =   10950
   ...
   Begin VB.ComboBox cmbSt 
      Height          =   360
      ItemData        =   "INDRegistroProducaoConsulta.frx":5C12
      Left            =   5040
      List            =   "INDRegistroProducaoConsulta.frx":5C1C
      TabIndex        =   15
      Top             =   1200
      Width           =   1815
   End
   Begin VB.CommandButton cmdUsu 
      ...
      Height          =   350
      Left            =   6480
      Picture         =   "INDRegistroProducaoConsulta.frx":5C31
      Style           =   1  'Graphical
      TabIndex        =   14
   ...
   Begin VB.ComboBox cmbEstab 
      Height          =   360
      ItemData        =   "INDRegistroProducaoConsulta.frx":6633
      Left            =   6960
      List            =   "INDRegistroProducaoConsulta.frx":663D
      TabIndex        =   13
      Top             =   1200
      Width           =   2055
   End
   ...
   Begin VB.CommandButton cmdProd 
      ...
      Height          =   350
      Left            =   960
      Picture         =   "INDRegistroProducaoConsulta.frx":6652
      Style           =   1  'Graphical
      TabIndex        =   11
      ToolTipText     =   "Consulta Produto"
      Top             =   2280
   ...
   Begin VB.ComboBox cmbEstabAUX 
      BackColor       =   &H80000003&
      Height          =   360
      ItemData        =   "INDRegistroProducaoConsulta.frx":7054
      Left            =   6960
      List            =   "INDRegistroProducaoConsulta.frx":705E
      TabIndex        =   9
      Top             =   1200
      Visible         =   0   'False
      Width           =   855
      ...
      Begin MSComctlLib.ImageList ImageList2 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "INDRegistroProducaoConsulta.frx":7073
               Key             =   ""
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "INDRegistroProducaoConsulta.frx":820D
               Key             =   ""
            EndProperty
            BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "INDRegistroProducaoConsulta.frx":929C
               Key             =   ""
            EndProperty
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "INDRegistroProducaoConsulta.frx":A3A7
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   End
End
Attribute VB_Name = "frmProducaoRegistroProducaoConsulta"
...
'On Error GoTo ERRO_TRATA
   ...
   SQL = "select REGISTROPRODUCAO.*, REGISTROPRODUCAOITEM.SEQ_ID, REGISTROPRODUCAOITEM.PRODUTO_ID, "
   SQL = SQL & " REGISTROPRODUCAOITEM.QTDE, REGISTROPRODUCAOITEM.valor, "
   ...
   SQL = SQL & " from REGISTROPRODUCAOITEM WITH (NOLOCK)"
   ...
   SQL = SQL & " INNER JOIN REGISTROPRODUCAO WITH (NOLOCK)"
   ...
   If PRODUTO_ID_N > 0 Then _
      SQL = SQL & " and REGISTROPRODUCAOITEM.PRODUTO_ID = " & PRODUTO_ID_N

   If Trim(cmbFamiliaAUX.Text) <> "" Then _
      SQL = SQL & " and familiaproduto_ID = " & cmbFamiliaAUX.Text

   SQL = SQL & " order by REGISTROPRODUCAO.REGISTROPRODUCAO_id, REGISTROPRODUCAOITEM.PRODUTO_ID"

   TabConsulta.Open SQL, CONECTA_RETAGUARDA, , , adCmdText
   While Not TabConsulta.EOF
      CONT_N = CONT_N + 1
      Set item = lstPerda.ListItems.Add(, "A" & CONT_N, TabConsulta.Fields("REGISTROPRODUCAO_id").Value)

      item.SubItems(1) = "" & TabConsulta.Fields("seq_id").Value
      item.SubItems(2) = "" & Trim(TabConsulta.Fields("codg_produto").Value)
      item.SubItems(3) = "" & Trim(TabConsulta.Fields("descricao").Value)
      item.SubItems(4) = "" & Format(TabConsulta.Fields("qtde").Value, strFormatacao3Digitos)
      item.SubItems(5) = "" & Format(TabConsulta.Fields("valor").Value, strFormatacao3Digitos)
      item.SubItems(6) = "" & Trim(TabConsulta.Fields("status").Value)
      item.SubItems(7) = "" & Trim(TabConsulta.Fields("descfamilia").Value)
...
'On Error GoTo ERRO_TRATA
   ...
   If Trim(txtDtIni.Text) <> "" And Trim(txtDtFim.Text) <> "" Then
      ...
      If optAbertura.Value = True Then
         FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.dt_registro} >= " & "DateTime(" & Year(DATA_INI) & "," & Month(DATA_INI) & "," & Day(DATA_INI) & "," & Hour(DATA_INI) & "," & Minute(DATA_INI) & "," & Second(DATA_INI) & ")"
         FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.dt_registro} <= " & "DateTime(" & Year(DATA_FIM) & "," & Month(DATA_FIM) & "," & Day(DATA_FIM) & "," & Hour(DATA_FIM) & "," & Minute(DATA_FIM) & "," & Second(DATA_FIM) & ")"
      End If
      If optFechamento.Value = True Then
         FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.dt_fecha} >= " & "DateTime(" & Year(DATA_INI) & "," & Month(DATA_INI) & "," & Day(DATA_INI) & "," & Hour(DATA_INI) & "," & Minute(DATA_INI) & "," & Second(DATA_INI) & ")"
         FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.dt_fecha} <= " & "DateTime(" & Year(DATA_FIM) & "," & Month(DATA_FIM) & "," & Day(DATA_FIM) & "," & Hour(DATA_FIM) & "," & Minute(DATA_FIM) & "," & Second(DATA_FIM) & ")"
      End If
   End If

   If Trim(cmbSt.Text) <> "" Then _
      FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.status} = '" & Left(cmbSt.Text, 1) & "'"

   If Trim(txtUsuCodg.Text) <> "" Then _
      FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.usuario_id} = " & txtUsuCodg.Text

   If PRODUTO_ID_N > 0 Then _
      FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAOitem.produto_id} = " & PRODUTO_ID_N

   If Trim(cmbFamiliaAUX.Text) <> "" Then _
      FORMULA_REL = FORMULA_REL & " and {REGISTROPRODUCAO.familiaproduto_ID} = " & cmbFamiliaAUX.Text
