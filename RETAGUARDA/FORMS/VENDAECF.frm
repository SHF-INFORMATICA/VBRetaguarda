...
'On Error GoTo ERRO_TRATA
   ...
   If (App.PrevInstance) Then
       ...
       nome_tela = App.Title
       App.Title = "Já estou em execução, frmVENDAECF !!!"
...
Private Sub Timer1_Timer()
   Me.Caption = "Venda de Produtos  |  " & ESTACAO_CPU & "  |  " & Time & "  |  Pedido: " & PEDIDO_ID_N & "  |  Cupom: " & NUMR_CUPOM_ABERTO
End Sub
...
'On Error GoTo ERRO_TRATA
   ...
   MOSTRA_MSG "Produto/Barras", "F2 - Incluir Cliente", "F3 - Cancela Venda", "F7 - Consulta Produtos", "F11 - Abre Gaveta"
...
'On Error GoTo ERRO_TRATA
   ...
   If KeyAscii = 13 Then
      ...
      PROCURA_PRODUTO txtProduto.Text

      txtQtde.Enabled = True

      If Indr_Achou_Produto = True Then _
...
'On Error GoTo ERRO_TRATA
   ...
   SQL = "SELECT PESSOA.CNPJCPF, PESSOA.DESCRICAO"
...
'On Error GoTo ERRO_TRATA
   ...
   SQL = SQL & " PEDIDO.TIPOVENDA_ID, PEDIDO.CGCCPF,"
   SQL = SQL & " PEDIDO.usuario_id, PEDIDO.DT_REQ, PEDIDO.STATUS as SituaçãoPedido, PEDIDO.TIPO_REGISTRO,"
   ...
   SQL = SQL & " PEDIDOITEM.VALOR_ITEM, PEDIDOITEM.CFOP_id, PEDIDOITEM.STRIBUTARIA, PEDIDOITEM.VALOR_DESCONTO AS DescontoItem,"
   ...
   SQL = SQL & " PRODUTO.SITUACAO as SituaçãoProduto, "
   ...
   While Not TabTemp.EOF
      ...
      Set Item = lstECF.ListItems.Add(, "seq." & TabTemp.Fields("SEQ_ID").Value, TabTemp.Fields("CODG_PRODUTO").Value)
...
'On Error GoTo ERRO_TRATA
   ...
   lblPedido.Caption = "Pedido : " & PEDIDO_ID_N
   lblTabela.Caption = "Estabelecimento: " & EMPRESA_ID_N

   MOSTRA_MSG "Venda de Mercadoria ", ESTACAO_CPU, "", "", ""
...
'On Error GoTo ERRO_TRATA
   ...
   MOSTRA_MSG "Abrindo Gaveta", ESTACAO_CPU, "", "", ""
...
ABRINDO_CUPOM_FISCAL:
      ...
      MOSTRA_MSG "Aguarde", "Abrindo Cupom Fiscal", "", "", ""

      If CNPJCPF_CLIENTE = "99999999999" Then
         RETORNO_ECF = Bematech_FI_AbreCupom("")
         Else: RETORNO_ECF = Bematech_FI_AbreCupom(CNPJCPF_CLIENTE)
      End If
      ...
      If Indr_Erro = True Then
         ...
         If CONTA_TENTATIVA >= 3 Then
            ...
            End If
            Else
               MOSTRA_MSG "Abrindo cupom fiscal ", "Tentativas = " & CONTA_TENTATIVA & " ; Erro = " & Msg, "", "", ""
      ...
      If Indr_Erro = True Then
         MOSTRA_MSG "Erro na leitura do Cupom Fiscal, Cupom Fiscal = " & NUMR_CUPOM_ABERTO, "", "", "", ""
         Exit Sub
      End If
      ...
      SQL = "insert into PEDIDO "
         ...
         SQL = SQL & " TIPOVENDA_ID,CGCCPF,"
         SQL = SQL & " usuario_id,DT_REQ,STATUS,TIPO_REGISTRO,NOME_CLIENTE,"
         SQL = SQL & " NUMERO_CAIXA_CPU,estabelecimento_id"
         SQL = SQL & " )"
      SQL = SQL & " values("
      ...
      GRAVA_CUPOM Str(PEDIDO_ID_N), NUMEROCUPOM
   End If
   Indr_Erro = False

Exit Sub
ERRO_TRATA:
   TRATA_ERROS Err.Description, Me.Name, "ABRE_CUPOM_FISCAL"
End Sub

Sub IMPRIME_ITEM_CUPOM_BEMATECH()
...
If Trim(txtProduto.Text) <> "" Then
   ...
   Aliquota_N = "01"
   SITUACAO_TRIBUT_A = lblST.Caption

   '1-Tributado;2-Isento;3-Outros;4-Base Calculo Reduzido;5-Diferido
   If SITUACAO_TRIBUT_A = "00" Then
      Aliquota_N = "01"
      ...
      SQL = SQL & " and aliquota = " & Aliquota_Icms_N
      TabUSU.Open SQL, CONECTA_RETAGUARDA, , , adCmdText
      If Not TabUSU.EOF Then _
...
TENTATIVAS:
   ...
   MOSTRA_MSG "Aguarde...", " Imprimindo " & Trim(txtDescricao.Text), "", "", ""

   RETORNO_ECF = Bematech_FI_VendeItem(Replace(Trim(Left(txtProduto.Text, 13)), ",", "."), _
      Trim(Replace(Left(Trim(txtDescricao.Text), 29), ",", ".")), _
      Trim(Aliquota_N), _
      UCase("f"), _
      ...
      "$", _
      Replace(Format$(VALOR_DESCONTO_N, strFormatacao2Digitos), ",", ".") _
      )

   Msg = ""
   Call VerificaRetornoImpressora("Bematech_FI_VendeItem", "", "Emissão de Cupom Fiscal")
   MOSTRA_MSG "Aguarde...", " Imprimindo " & Trim(txtDescricao.Text), "", "", ""
'==============================================
'=================================================
   If Indr_Erro = True Then
      If CONTA_TENTATIVA >= 1000 Then
         If Indr_Erro = True Then
            MsgBox "Erro ao imprimir produto = " & Trim(TabPedidoItem.Fields("CODG_PRODUTO").Value) & " - " & TabPedidoItem.Fields("descricao").Value & " , verificar. 01"
            Call frmINICIO.EasyTEF.CancelarVendasPendentes
            GRAVANDO_ERRO_EMISSAO_CUPOM
         End If
         Else
            Me.Caption = "Imprimindo, Tentativas = " & CONTA_TENTATIVA & " ; Erro  "
Sleep 1000
            ...
            If RESPOSTA = vbYes Then
               ...
               GoTo TENTATIVAS
               Else
                  MsgBox "Erro ao imprimir produto = " & Trim(TabPedidoItem.Fields("CODG_PRODUTO").Value) & " - " & TabPedidoItem.Fields("descricao").Value & " , verificar. 02"
                  Call frmINICIO.EasyTEF.CancelarVendasPendentes
                  GRAVANDO_ERRO_EMISSAO_CUPOM
            End If
      End If
...
'=================================================
   ...
   If TabUSU.EOF Then
      SQL = "insert into PEDIDO "
         ...
         SQL = SQL & " PEDIDO_ID,CLIENTE_ID,EMPRESA_ID,VENDEDOR_ID,"
         SQL = SQL & " TIPOVENDA_ID, CGCCPF,"
         SQL = SQL & " usuario_id,DT_REQ,STATUS,TIPO_REGISTRO,"
         SQL = SQL & " NOME_CLIENTE,NUMERO_CAIXA_CPU,estabelecimento_id"
         SQL = SQL & " )"
      SQL = SQL & " values("
         SQL = SQL & PEDIDO_ID_N                       'PEDIDO_ID
         SQL = SQL & "," & CLIENTE_ID_N               'CLIENTE_ID
   ...
   SQL = "insert into PEDIDOITEM "
      SQL = SQL & " ("
         ...
         SQL = SQL & " VALOR_ITEM,STRIBUTARIA,STATUS,TIPO_REG "
      SQL = SQL & " )"
   SQL = SQL & " values("
      SQL = SQL & PEDIDO_ID_N                          'PEDIDO_ID
...
Exit Sub
ERRO_TRATA:
   TRATA_ERROS Err.Description, Me.Name, "IMPRIME_ITEM_CUPOM_BEMATECH"
   INDR_VENDA = False
   CRITERIO = ""
End Sub
...
'On Error GoTo ERRO_TRATA
   ...
   If Not TabLancamento.EOF Then
      ...
      Else
         ...
         SQL = SQL & " VALUES ("
            SQL = SQL & USUARIO_ID_N                                                                                'Usu_Alt
            SQL = SQL & ",'" & DMA(SqL2) & "'"                                                                    'Dt_Alt
            SQL = SQL & "," & NUMR_ID_N                                                                           'Lancamento_id
            SQL = SQL & "," & PEDIDO_ID_N                                                                          'Numr_doc
            SQL = SQL & "," & PEDIDO_ID_N                                                                          'NUMR_DP
...
'On Error GoTo ERRO_TRATA
   ...
   TabTemp.Open SQL, CONECTA_RETAGUARDA, , , adCmdText
   If Not TabTemp.EOF Then
      txtValorRecebido.Text = Format(TabTemp.Fields(0).Value, strFormatacao2Digitos)
      txtValorCompra.Text = Format(TabTemp.Fields(0).Value, strFormatacao2Digitos)
      txtValorTotal.Text = Format(TabTemp.Fields(0).Value, strFormatacao2Digitos)
   End If
   If TabTemp.State = 1 Then _
      TabTemp.Close
...
'On Error GoTo ERRO_TRATA
   ...
   If Not IsNull(PEDIDO_ID_N) Then
      If PEDIDO_ID_N > 0 Then
...
'===========
   ...
   If Trim(UCase(NOME_CLIENTE)) <> UCase("Consumidor Final") Then
      SQL = Trim(Left(NOME_CLIENTE, 48))
